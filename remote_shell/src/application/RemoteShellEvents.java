...
public class RemoteShellEvents extends RemoteShellScreen implements EventsInterface {
	...
	public void buttonSettingClick(ActionEvent ev) {
		Thread setar = new Thread(() -> {
			if (!getTextFieldUser().getText().isEmpty() && !getTextFieldUser().getText().isEmpty()) {
				...
				if (!AUTH_FILE.getArq().isFile())
					JOptionPane.showMessageDialog(null, TEXT.pegarTexto(20), TITLE, JOptionPane.INFORMATION_MESSAGE,
							IMAGE.pegarIcon("/resource/shell.png"));
				String set = CMD.comando(FOLDER_SH.getCaminho() + "set " + getTextFieldUser().getText() + " "
						+ getTextFieldEmail().getText());
				getLabelInformation().setText(set);
				CMD.sleep(5);
				if (FOLDER_REPO.getArq().isDirectory()) {
					JOptionPane.showMessageDialog(null, TEXT.pegarTexto(13), TITLE, JOptionPane.INFORMATION_MESSAGE,
							IMAGE.pegarIcon("/resource/shell.png"));
	...
	public void buttonResetClick(ActionEvent ev) {
		Thread resetar = new Thread(() -> {
			int choose = JOptionPane.showConfirmDialog(null, TEXT.pegarTexto(11), TITLE, 0,
					JOptionPane.QUESTION_MESSAGE, IMAGE.pegarIcon("/resource/shell.png"));
			if (choose == 0) {
				getLabelInformation().setText(TEXT.pegarTexto(17));
				CMD.comando(FOLDER_SH.getCaminho() + "reset");
				if (!FOLDER_REPO.getArq().isDirectory()) {
					JOptionPane.showMessageDialog(null, TEXT.pegarTexto(15), TITLE, JOptionPane.INFORMATION_MESSAGE,
							IMAGE.pegarIcon("/resource/shell.png"));
					getLabelInformation().setText(TEXT.pegarTexto(14));
					CMD.sleep(3);
					buttonExitClick(ev);
